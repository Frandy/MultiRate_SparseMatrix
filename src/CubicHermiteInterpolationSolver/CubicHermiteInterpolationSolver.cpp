...
void CubicHermiteInterpolationSolver::setInput(const CubicHermiteInterpolationInputSolver& input) {
	this->input = input;
}

const CubicHermiteInterpolationOutputSolver& CubicHermiteInterpolationSolver::getOutput() const {
	return output;
}
	...
	for (Integer i = 0; i < ref_size; ++i){
		if (ref(i) == 0){
			if (t1 < t1_int(i)){
				...
				Real r2 = (t2 - t1_int(i))/((1-gm) * h_int(i));

				y1(i) = (v3 - 2*v2) * (r1*r1*r1)  + (3*v2 - v3) * (r1*r1) + v1*r1 + v0;
				y2(i) = (v3 - 2*v2) * (r2*r2*r2)  + (3*v2 - v3) * (r2*r2) + v1*r2 + v0;
			}
			if(t2 < t1_int(i)){
				Real v0 = y0_int(i);
				Real v1 = gm * z0_int(i);
