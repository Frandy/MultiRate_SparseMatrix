...
void MultiRateTRBDF2Solver::compute(){
	...
	CubicHermiteInterpolationInputSolver interp_input;	//input for the interpolator

	std::vector<CubicHermiteInterpolationInputSolver> interp_input_vec;

	Bool newton_converge;

	unsigned n_sub = 0;             // sub-levels index
			...
			if ( newton_converge == true ){
				...
				if( refining == true ){
					...
					if(n_sub == 0){
						...
						//become more than one, we could have different times slab where the solution
						//is knowed.
						...
						z0_temp += VectorUtility::vectorProlong(z0,ref_vec[n_sub-1]);
						...
						z1_temp += VectorUtility::vectorProlong(z1,ref_vec[n_sub-1]);
						...
						z2_temp += VectorUtility::vectorProlong(z2,ref_vec[n_sub-1]);
						...
						y0_temp += y0.cwiseProduct(ref_vec[n_sub-1].cast<Real>());
						...
						y1_temp += y1.cwiseProduct(ref_vec[n_sub-1].cast<Real>());
						...
						y2_temp += y2.cwiseProduct(ref_vec[n_sub-1].cast<Real>());
						...
						t0_temp += t0*((ref_vec[n_sub-1]).cast<Real>());
						...
						t1_temp += t1*((ref_vec[n_sub-1]).cast<Real>());
						...
						t2_temp += t2*((ref_vec[n_sub-1]).cast<Real>());
					...
					n_sub = n_sub + 1;
					hz_vec[n_sub] =(hz_vec[n_sub-1].cwiseProduct(ref_vec[n_sub-1].cast<Real>()));
